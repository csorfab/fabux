...
function UndoRedo() {
  ...
  const doUndo = useCallback(() => {
    ...
    if (undidAction) {
      setRedoableActions((redoableActions) => [
        ...redoableActions,
        undidAction
      ]);
    }
  ...
  return (
    <>
      ...
      <input
        ...
        value={name}
        onChange={(e) =>
          dispatch({ type: "updateName", name: e.currentTarget.value })
        }
      />
    </>
  );
...
function TodoInfo() {
  ...
  const notCompletedCount = useSelector(
    (state: AppState) => state.todos.length + addState
  );
